...
void *bd_malloc(uint64 nbytes) {
  ...
  if (k < MAXSIZE) {
    bit_invert(bd_sizes[k + 1].alloc, blk_index(k, p) / 2);
  }
  ...
  for (; k > fk; k--) {
    ...
    bit_invert(bd_sizes[k].alloc, blk_index(k - 1, p) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_invert(bd_sizes[k + 1].alloc, bi / 2);
    if (bit_isset(bd_sizes[k + 1].alloc, buddy / 2)) {  // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, int code) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (k < MAXSIZE && bit_isset(bd_sizes[k + 1].alloc, bi / 2)) {
    free = BLK_SIZE(k);
